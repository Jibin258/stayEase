...
const IconSlider = () => {
    ...
    return (
        <div className="relative">
            <h3 className='text-xl font-semibold my-5 text-slate-500'>
                Our Offerings & Amenities
            </h3>

            <button className="absolute left-[-3.5%] top-1/2 transform -translate-y-1/2 text-2xl font-bold opacity-75 hidden md:block" onClick={scrollLeft}>
                ❮
            </button>
                ...
                <div ref={scrollRef} className="flex space-x-4 overflow-x-auto md:overflow-x-hidden h-[28vh] lg:h-[30vh] pt-5 px-2 lg:px-3">
                    {icons.map((element) => (
                        <div key={element.id} className='relative'>
                            <div className="relative w-[7rem] md:w-[10vw] lg:w-[7vw] h-[13vh] lg:h-[14vh] rounded-lg hover:bg-[#f7b600] hover:text-white transition duration-500 text-center shadow-custom">
                                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-4 bg-[#fff2f0] rounded-full size-20">
                                    <span className="text-[#f7b600] text-5xl">{element.icon}</span>
                                </div>
                            </div>
                            <div className="absolute top-[73%] lg:top-[75%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center text-gray-500">{element.iconName}</div>
                        </div>
                    ))}
                </div>
            </div>

            <button className="absolute right-[-3.5%] top-1/2 transform -translate-y-1/2 text-2xl font-bold opacity-75 hidden md:block" onClick={scrollRight}>
                ❯
            </button>
        </div>
    )
